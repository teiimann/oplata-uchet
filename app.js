(() => {
  ...
  function overview() {
    ...
    content += `<section class="section"><div class="section-title"><h2>Ожидают оплату</h2><span class="muted">Счета: ${unpaid.length}</span></div>${unpaid.length ? `<div class="panel">${unpaid.map(row).join('')}</div>` : empty('✓', invoices.length ? 'Все счета оплачены' : 'Пока нет начислений', invoices.length ? 'За этот месяц задолженности нет.' : 'Добавьте плательщика и создайте начисления.', !data.clients.length ? `<button class="btn btn-primary" data-action="add-client">Добавить плательщика</button>` : '')}</section>`;
    if (invoices.length && invoices.length !== unpaid.length) content += `<section class="section"><div class="section-title"><h2>Оплачено полностью</h2><span class="muted">Счета: ${invoices.length - unpaid.length}</span></div><div class="panel">${invoices.filter(i => invoiceDue(i) <= 0).map(row).join('')}</div></section>`;
    return content;
  }
